...
<form action="/edit_blog_save" method="post" class="w-full" enctype="multipart/form-data">
    <input type="hidden" name="id_blog" id="id_blog" class="" value="<?= $blog['id_blog']; ?>">
    <input type="hidden" name="gambarLama" value="<?= $blog['foto']; ?>">
    ...
    <div class="flex items-center justify-center">
        <div class="w-36">
            <img src="/img/<?= $blog['foto']; ?>" class="img-preview object-cover">
        </div>
    </div>
    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="gambar">Gambar:</label>
    <input onchange="previewImg()" class="custom-file-input mb-3 block w-full text-sm text-gray-900 border border-gray-300 rounded-sm cursor-pointer bg-gray-50" value="<?= $blog['foto']; ?>" id="gambar" name="gambar" type="file">


    <label for="judul" class="block mb-2 text-sm font-medium text-gray-900">Judul:</label>
    <input type="text" name="judul" id="judul" value="<?= $blog['judul'] ?>" class="bg-gray-50 mb-3 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">

    <label for="isi" class="block mb-2 text-sm font-medium text-gray-900">Isi:</label>
    <input type="text" name="isi" id="isi" value="<?= $blog['isi'] ?>" class="bg-gray-50 border mb-3 border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
