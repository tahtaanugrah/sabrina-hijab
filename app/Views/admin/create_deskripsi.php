...
<form action="/tambah_deskripsi_save" method="post" class="w-full" enctype="multipart/form-data">
    ...
    <select class="mb-3" id="jenis" name="namajenis">
        ...
        <?php foreach ($jenis as $index => $k) : ?>
            <option value="<?= $k['id_jenis']; ?>" <?= old('namajenis') === $k['id_jenis'] ? 'selected' : '' ?>><?= $k['namajenis']; ?></option>
        <?php endforeach ?>
    </select>

    <label for="warna" class="block mb-2 text-sm font-medium text-gray-900">warna:</label>
    <input type="text" name="warna" id="warna" value="<?= old('warna') ?>" class="bg-gray-50 mb-3 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
    <label for="deskripsi" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi:</label>
    <textarea name="deskripsi" id="deskripsi" class="w-full" cols="30" rows="5"><?= old('deskripsi') ?></textarea>
