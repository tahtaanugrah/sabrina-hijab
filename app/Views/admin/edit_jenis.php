...
<form action="/edit_jenis_save" method="post" class="w-full" enctype="multipart/form-data">
    <input type="hidden" name="id_jenis" id="id_jenis" class="" value="<?= $jenis['id_jenis']; ?>">
    <input type="hidden" name="gambarLama" value="<?= $jenis['foto']; ?>">
    ...
    <div class="flex items-center justify-center">
        <div class="w-36">
            <img src="/img/<?= $jenis['foto']; ?>" class="img-preview object-cover">
        </div>
    </div>
    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="gambar">Gambar:</label>
    <input onchange="previewImg()" class="custom-file-input mb-3 block w-full text-sm text-gray-900 border border-gray-300 rounded-sm cursor-pointer bg-gray-50" value="<?= $jenis['foto']; ?>" id="gambar" name="gambar" type="file">

    <label for="katalog">Katalog:</label>
    <select class="mb-3" id="katalog" name="namakatalog">
        <option value="">Pilih katalog</option>
        <?php foreach ($katalog as $index => $k) : ?>
            <option value="<?= $k['id_katalog']; ?>" <?= $jenis['id_katalog'] === $k['id_katalog'] ? 'selected' : '' ?>><?= $jenis['id_katalog'] === $jenis['id_katalog'] ? $k['namakatalog'] : '' ?></option>
        <?php endforeach ?>
    </select>

    <label for="namajenis" class="block mb-2 text-sm font-medium text-gray-900">Nama Jenis:</label>
    <input type="text" name="namajenis" id="namajenis" value="<?= $jenis['namajenis'] ?>" class="bg-gray-50 mb-3 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
